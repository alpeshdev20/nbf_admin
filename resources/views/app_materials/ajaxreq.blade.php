...
<?php foreach($indeces as $i){ ?>
	$(<?= "'#genre_id$i'" ?>).prepend('<option value="">None</option>').val(<?= $values['genre_id'.$i] ?>);
	refreshDepartments(<?= e2q($i) ?>,<?= $values['genre_id'.$i] ?>,<?= $values['department_id'.$i] ?>);
	refreshSubjects(<?= e2q($i) ?>,<?= $values['department_id'.$i] ?>,<?= $values['subject_id'.$i] ?>);
<?php } ?>	
